package sanchez.sanchez.sergio.agrociety.ui.features.main.components.bottomnav
...
class CustomBottomNavigationView @JvmOverloads constructor(
    ...
    init {
        Timber.d("CustomBottomNavigationView init called")
        attrs?.let { init(it) }
    }

    private fun init(attrs: AttributeSet) {

        Timber.d("Init Component, Configure attrs")
    ...
    fun changeCenterButtonIcon(icon: Int) {
        if (centreButton == null) {
            Timber.d("You should call setCentreButtonIcon() instead, changeCenterButtonIcon works if space navigation already set up")
        } else {
    ...
    fun changeSpaceBackgroundColor(@ColorInt color: Int) {
        if (color == spaceBackgroundColor) {
            Timber.d("changeSpaceBackgroundColor: color already changed")
            return
        }
    ...
    private fun updateSpaceItems(selectedIndex: Int) {
        ...
        for (i in spaceItemList.indices) {
            if (i == selectedIndex) {
                ...
                val spaceItemIcon = textAndIconContainer.findViewById(sanchez.sanchez.sergio.agrociety.R.id.space_icon) as ImageView
                val spaceItemText = textAndIconContainer.findViewById(R.id.space_text) as TextView
                spaceItemText.setTextColor(activeSpaceItemColor)
                spaceItemIcon.changeImageViewTint(activeSpaceItemColor)
    ...
    private fun restoreBadges() {
        ...
        if (restoredBundle != null) {
            ...
            if (restoredBundle.containsKey(BADGES_ITEM_BUNDLE_KEY)) {
                badgeSaveInstanceHashMap =  savedInstanceState?.getSerializable(BADGES_ITEM_BUNDLE_KEY) as HashMap<Int, Any>
